...
Boat::Boat(const std::string& name, Bank* bank) : Container(name),
                                                  _currentBank(bank) {}
...
bool Boat::hasDriver() const {
   return std::find_if(_onBoard.begin(), _onBoard.end(),
                       [](const Person* p) { return p->canDrive(); }) !=
          _onBoard.end();
}
...
bool Boat::add(const Person* p) {
   if (_onBoard.size() == MAX_CAPACITY) {
      std::cout << ERROR_PREFIX << " Il n'y a plus de place dans le bateau" << std::endl;
      return false;
   }

   if (std::find(_onBoard.begin(), _onBoard.end(), p) != _onBoard.end()) {
      std::cout << ERROR_PREFIX << p->getName() << "est déjà dans le bateau"
                << std::endl;
      return false;
   }

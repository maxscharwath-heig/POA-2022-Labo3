...
void tests() {
   ...
   std::cout << cp.getName() << " " << ucp.getName() << " " << dad->getName() << std::endl;
   ...
   Bank bank("bankname");
   if (bank.add(&cp)) {
      std::cout << "Should be 1: " << (bank.getPeople().back() == &cp) << std::endl;
   }
   else {
   ...
   std::cout << "Should be 1: " << (bank2.getPeople().size() == shouldPassCheck.size()) << std::endl;
